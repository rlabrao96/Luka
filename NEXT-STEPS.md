...
- **Budget config modal on /budgets** — A single accordion modal triggered by a gear button next to the currency toggle on `/budgets`, consolidating the 5 budget knobs (savings target, payday, personal allocation, contribution mode, per-category caps) that used to live scattered across `/settings`. The category-caps editor is redesigned: empty-by-default list of active caps + a `+ Agregar tope` picker that surfaces the top-5 spent categories from the current month's `budget-v2` Sankey data (no more 20 empty rows). Each row has its own per-row save mutation with a 900ms auto-collapse and a "Guardado ✓" chip. Mobile becomes a bottom sheet via the existing `animate-slide-up` keyframe. Backend shim: `/auth/me` now also returns the caller's `contribution_mode` / `fixed_contribution_amount` / `fixed_contribution_currency` fields (PATCH and GET both populate them) — resolves a pre-existing TODO where the old `ContributionSection` always defaulted to `mode='full'` on mount because there was no GET path for the value. Old `BudgetSettingsSection`, `ContributionSection`, `CategoryBudgetsSection` deleted. Spec: `docs/superpowers/specs/2026-04-15-budget-inline-config-design.md`. Plan: `docs/superpowers/plans/2026-04-15-budget-inline-config-plan.md`.
