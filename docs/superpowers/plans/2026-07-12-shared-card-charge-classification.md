...
and replace `t.user_id` in the payer JOIN/GROUP BY with `COALESCE(a.attributed_to_user_id, t.user_id)`. Keep the `split_type='shared'` filter and the totals-exclusion (now also excludes `needs_classification`). Do NOT change income/personal blocks. The five blocks are the queries in `get_contribution_summary`, `get_category_breakdown`, `get_settlement` (inner subquery), `get_equity_report` (totals query), `get_member_stats`. **NOTE — `get_member_stats` is structurally different:** it credits the payer via the LEFT-JOIN ON-clause `t.user_id = u.id`, NOT a `GROUP BY t.user_id`. There, the rewrite means the ON-clause becomes `COALESCE(a.attributed_to_user_id, t.user_id) = u.id` with the `transaction_attributions` LEFT JOIN added in the same scope — don't just swap a GROUP BY. The per-payer tests in Step 1 will catch a miss.
