...
| `transaction_attributions` | Partner-card charge hand-off (migration 054). One row per transaction (UNIQUE). `attributed_to_user_id` (recipient) / `attributed_by_user_id` (sender) / `status` (`active`\|`rejected`) / `acknowledged_at`. An **active** row means the charge counts for the recipient, not the card owner — used for a shared authorized-user card (e.g. a spouse's additional Amex). The row's presence is what distinguishes a handed-off `split_type='partner'` transaction from an account-level exclude-only one. | -> transactions, users |
| `bank_accounts` | Connected accounts with sync state. `account_type` includes `shared_card` (migration 056) — a credit card physically shared by two household members with no way to tell whose charge is whose at ingestion (unlike `joint`, which auto-classifies everything `shared`, or `partner`, which is a Plaid-level authorized-user card resolved via `transaction_attributions`). | -> households, users |
...
### Shared-card charge classification (migration 056)

A `shared_card` bank account is a physically-shared card (e.g. one household Visa two partners both carry) where — unlike a `joint` account (auto-classified `shared`) or a `partner` authorized-user card (resolved via `transaction_attributions` hand-off) — there is no signal at ingestion time for whose charge is whose. Every non-transfer charge lands with `needs_classification=True` (`should_pend` in `modules/transactions/classification.py`; CC bill-payment transfers are never pended) and is invisible to every money aggregate (totals, budgets, settlement, per-account balance) until sorted. Both members see the same pending queue — `GET /transactions/por-clasificar?household_id=` — and either can sort a charge four ways via `POST /transactions/{id}/classify` (`outcome`: `owner_personal` / `partner_personal` / `owner_shared` / `partner_shared`, optional `partner_id` for >2-member households). Sorting is first-to-sort-wins: a charge whose `needs_classification` already flipped to `False` returns 409 `already_classified`. `owner_shared`/`partner_shared` sets `split_type='shared'`; the `partner_*` outcomes additionally write an active `transaction_attributions` row so the effective-payer resolves to the partner (see the money-invariant note below). Switching an account's type TO `shared_card` pends its existing non-transfer history (no split backfill); switching AWAY clears `needs_classification` and runs the normal split backfill for the new type (`modules/bank_accounts/router.py:update_bank_account`). A daily cron (`modules/notifications/pending_classification.py`) notifies every active household member once per day while any charge is pending, idempotent per user/day. **Leave-household edge:** when a member leaves, `remove_member` calls `resolve_pending_to_owner_on_leave` (`modules/transactions/classification.py`) — every still-pending shared-card charge in that household resolves to `owner_personal` (split reverted to `personal`, any attribution deleted, `needs_classification=False`), since the classification queue is a two-person surface and nothing should stay pending against a departed partner.
...
### Trips (Viajes) tables (migrations 045-050)
...
**Mutual exclusivity invariant (migrations 048 + 049):** A transaction cannot simultaneously carry a **shared** household split (`transaction_splits` row with `split_type='shared'`) and a trip-expense link (`trip_expenses.transaction_id`). Two BEFORE INSERT/UPDATE triggers on `transaction_splits` and `trip_expenses` enforce this; violators raise a SQL exception that surfaces as HTTP 409 `joint_account_dual_split_not_supported` from the trip router. Migration 048 introduced the triggers; migration 049 narrowed both to `split_type='shared'` only — every Luka transaction has a default `personal` split row from `ensure_default_split`, so the over-broad 048 version blocked all trip linking, not just joint-account ones. v1 keeps shared (joint-account) splits and trip splits cleanly disjoint — dual-split (one transaction tagged to both surfaces) is deferred to v2. Personal split rows coexist freely with trip links.

**Trip → transactions FK cascade (migration 050).** All four trip-side FKs to `transactions.id` (`trip_expenses`, `trip_settlements`, `trip_suggestion_dismissals`, `trip_settlement_dismissals`) had no ON DELETE behavior in migration 046, so any user who linked a trip expense or dismissed a suggestion against a pending Plaid txn broke the next sync with `ForeignKeyViolationError`. Migration 050 sets `CASCADE` on the two dismissal tables and `SET NULL` on `trip_expenses` / `trip_settlements`. The Plaid sync's pending-replacement path (`modules/plaid/sync.py`) also re-points all four FKs to the replacement row before deleting the old txn, so user-attributed trip data follows the pending → settled rotation rather than getting orphaned by the cascade.
...
| POST | `/transactions/{id}/attribute` | Hand off a charge on a shared authorized-user card to a household partner (body `recipient_id?`). **Card-owner only** (`caller == txn.user_id`). Resolves the recipient automatically in a couple; `409 ambiguous_recipient` if >1 other member, `422 no_partner_in_household` if none. Sets `split_type='partner'` + creates an active `transaction_attributions` row + notifies the recipient. |
| DELETE | `/transactions/{id}/attribute` | Un-tag a handed-off charge. Sender only. |
| POST | `/transactions/attributions/{id}/reject` | Recipient rejects a handed-off charge → reverts to the sender's ledger + notifies the sender. Recipient-only (403 otherwise, 404 if missing). |
| POST | `/transactions/attributions/{id}/acknowledge` | Recipient confirms a handed-off charge (sets `acknowledged_at`; counting is unchanged — it already counts for the recipient). Recipient-only. |
| GET | `/transactions/por-clasificar?household_id=` | Shared-card charges awaiting classification (`needs_classification=True` on a `shared_card` account), visible identically to every active member. Any-member auth (403 non-members). |
| POST | `/transactions/{id}/classify` | Four-way sort of a pending shared-card charge — body `{outcome: owner_personal\|partner_personal\|owner_shared\|partner_shared, partner_id?}`. Any active household member may sort; first-to-sort wins (409 `already_classified` on a second call). `partner_id` disambiguates the recipient in >2-member households (409 `ambiguous_recipient` if omitted and ambiguous, 422 `no_partner_in_household` if the household has no other member). |
...
8. **Exactly-one-owner for partner-card attribution** (`backend/modules/transactions/attribution.py`). A transaction's amount counts toward exactly one person's personal totals: the recipient when it has an **active** `transaction_attributions` row, else the card owner — never both, never neither (account-level exclude-only `partner` rows, which have no attribution row, deliberately count for nobody). Three distinct predicates enforce this and must be reused, never re-derived: `owned_by_caller_clause` (+ NULL-safe `exclude_only_partner` guard) for aggregates (dashboard/category), `list_visible_clause` for list views (sender still sees handed-off rows, labeled), `personal_scope_clause` for the personal budget. All three require `outerjoin(TransactionAttribution)`. The guard MUST be NULL-safe (`coalesce(split_type,'')=='partner'`) — email rows have no split row, and a bare `== 'partner'` silently drops them.
9. **Shared-card settlement credits the effective payer, not the account owner.** Once a `shared_card` charge is classified `shared` (`owner_shared` or `partner_shared`), every settlement/contribution/breakdown aggregate credits `COALESCE(active attribution.attributed_to_user_id, transaction.user_id)` — the same `effective_payer_id` helper the partner-card attribution feature introduced, mirroring `effective_owner_id`. A `partner_shared` classification means the partner (not the card owner) fronted the money, so settlement owes the partner back, not the owner. A charge with `needs_classification=True` counts for **nobody** — it's excluded by the same totals-exclusion predicate (#2 above) until sorted, so an unsorted shared-card charge cannot silently distort anyone's balance either way.
