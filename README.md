...
- Alembic migrations (42 versions; `pg_trgm` extension enabled for fuzzy merchant matching). Auto-applied on every API container start (`alembic upgrade head` baked into the Dockerfile's `CMD`).
