...
- Alembic migrations (45 versions through `044_reimbursement_group_id`; `pg_trgm` extension enabled for fuzzy merchant matching). Auto-applied on every API container start (`alembic upgrade head` baked into the Dockerfile's `CMD`).
